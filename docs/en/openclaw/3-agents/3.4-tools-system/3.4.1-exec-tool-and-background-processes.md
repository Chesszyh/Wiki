...
```mermaid
flowchart TD
    ExecTool["exec tool(bash-tools.exec.js)"]
    ProcessTool["process tool(bash-tools.process.js)"]
    ...
    Sweeper["TTL Sweeper(setInterval)"]
...
```mermaid
flowchart TD
    Running["runningSessionsMap<string, ProcessSession>"]
    Finished["finishedSessionsMap<string, FinishedSession>"]
    PSFields["id: stringcommand: stringscopeKey?: stringsessionKey?: stringchild?: ChildProcessstdin?: SessionStdinpid?: numberstartedAt: numbermaxOutputChars: numberpendingStdout: string[]pendingStderr: string[]aggregated: stringtail: stringexitCode?: number | nullexited: booleantruncated: booleanbackgrounded: boolean"]
    FSFields["id: stringcommand: stringscopeKey?: stringstartedAt: numberendedAt: numberstatus: ProcessStatusexitCode?: number | nullaggregated: stringtail: stringtruncated: boolean"]
...
```
{  "status": "running",  "newOutput": "Building project...\n",  "tail": "..."}
```
**Returns (after exit):**

```
{  "status": "completed",  "exitCode": 0,  "newOutput": "Build complete.\n",  "aggregated": "...",  "tail": "..."}
```
...
```
{  "tool": "process",  "action": "write",  "sessionId": "abc123",  "data": "y\n"}
```
...
```
{  "tool": "process",  "action": "log",  "sessionId": "abc123",  "offset": 100,  "limit": 50}
```
