...
本文档对位于 [https://github.com/kubernetes/kubernetes](https://github.com/kubernetes/kubernetes) 的 Kubernetes 代码库进行高层介绍。它描述了基础架构、主要子系统，以及它们如何相互协作以形成完整的容器编排系统。

本概览聚焦于运行时组件及其在 monorepo 中的组织方式。关于特定子系统的详细信息：

-   核心 API 系统与特性门控：参见 [Core API System and Feature Management](/kubernetes/kubernetes/2-core-api-system-and-feature-management)
-   控制平面组件：参见 [Control Plane Components](/kubernetes/kubernetes/3-control-plane-components)
-   节点组件：参见 [Node Components](/kubernetes/kubernetes/4-node-components)
-   构建与测试基础设施：参见 [Build System and Dependencies](/kubernetes/kubernetes/7-build-system-and-dependencies)

---

## 仓库组织

Kubernetes 仓库是一个 monorepo，包含构建、测试和运行 Kubernetes 集群所需的全部组件。代码库按若干关键目录组织：

| Directory | Purpose |
| --- | --- |
| `cmd/` | 主要可执行程序（`kube-apiserver`、`kubelet`、`kube-proxy`、`kube-scheduler`、`kube-controller-manager`、`kubectl`） |
| `pkg/` | 所有组件的核心实现包 |
| `staging/src/k8s.io/` | 对外发布的模块（API 类型、客户端库），随后回灌到主仓库 |
| `test/` | 端到端与集成测试 |
| `build/` | 构建脚本与工具链 |
| `cluster/` | 集群置备脚本（GCE、本地集群等） |
| `api/` | 生成的 OpenAPI 规范 |

**来源：** 仓库结构，[staging/src/k8s.io/api/core/v1/types.go1-25](https://github.com/kubernetes/kubernetes/blob/2757a872/staging/src/k8s.io/api/core/v1/types.go#L1-L25) [cmd/kube-apiserver/app/server.go1-20](https://github.com/kubernetes/kubernetes/blob/2757a872/cmd/kube-apiserver/app/server.go#L1-L20)
...
```
Kubernetes 由控制平面和工作节点组成。控制平面负责管理集群状态，各组件通过 API server 协调，并将状态持久化到 etcd。工作节点组件（kubelet 与 kube-proxy）监听 API server 的变化，并据此对本地状态执行调谐。
...
API server 是 Kubernetes 的中心枢纽。它对外暴露 Kubernetes API，负责请求校验与处理，并将状态持久化到 etcd。

**关键代码实体：**

-   入口点：`cmd/kube-apiserver/app/server.go`
-   主服务初始化：`CreateServerChain()` 函数
-   存储后端：`staging/src/k8s.io/apiserver/pkg/storage/cacher/cacher.go`
-   API 类型：`staging/src/k8s.io/api/core/v1/types.go`

API server 使用委托链（delegation chain）模式，将多个 `GenericAPIServer` 实例串联起来以服务不同 API 组。

**来源：** [cmd/kube-apiserver/app/server.go60-120](https://github.com/kubernetes/kubernetes/blob/2757a872/cmd/kube-apiserver/app/server.go#L60-L120) [staging/src/k8s.io/apiserver/pkg/server/config.go1-50](https://github.com/kubernetes/kubernetes/blob/2757a872/staging/src/k8s.io/apiserver/pkg/server/config.go#L1-L50)

### kubelet

kubelet 运行在每个工作节点上，负责管理 Pod 生命周期。它监听 API server 的 Pod 分配，确保容器按预期运行，并回报状态。
...
-   主结构体：`pkg/kubelet.Kubelet`
-   初始化：`pkg/kubelet.NewMainKubelet()`
-   Pod 同步：`pkg/kubelet.syncPod()`
-   容器运行时：`pkg/kubelet/kuberuntime.kubeGenericRuntimeManager`
-   卷管理器：`pkg/kubelet/volumemanager.volumeManager`
...
kube-proxy 通过将 Service 与 EndpointSlice 对象转换为网络规则（iptables、IPVS 或 nftables），实现 Kubernetes Service 网络功能。
...
-   主入口：`cmd/kube-proxy/app.NewProxyCommand()`
-   iptables 模式：`pkg/proxy/iptables.Proxier`
-   IPVS 模式：`pkg/proxy/ipvs.Proxier`
-   同步逻辑：`pkg/proxy/iptables.syncProxyRules()`
-   Service 跟踪：`pkg/proxy.ServiceChangeTracker`
...
scheduler 根据资源需求、约束条件与基于插件的评分机制将 Pod 分配到节点。

**关键代码实体：**

-   主结构体：`pkg/scheduler.Scheduler`
-   调度周期：`pkg/scheduler.scheduleOne()`
-   框架：`pkg/scheduler/framework.Framework`
-   插件注册：`pkg/scheduler/framework/runtime.frameworkImpl`

**来源：** [pkg/scheduler/scheduler.go1-100](https://github.com/kubernetes/kubernetes/blob/2757a872/pkg/scheduler/scheduler.go#L1-L100)

---

## API 类型系统与校验

所有 Kubernetes 资源都定义为强类型 Go 结构体，并带有校验规则和特性门控集成。

### 核心 API 类型

API 对象模型定义在 `staging/` 目录，并以独立模块形式发布：

| Package | Purpose | Example Types |
| --- | --- | --- |
| `k8s.io/api/core/v1` | 核心资源类型 | `Pod`, `Service`, `Volume`, `Container` |
| `k8s.io/api/apps/v1` | 应用资源类型 | `Deployment`, `StatefulSet`, `DaemonSet` |
| `k8s.io/apimachinery/pkg/apis/meta/v1` | API machinery 类型 | `ObjectMeta`, `TypeMeta`, `ListMeta` |

**来源：** [staging/src/k8s.io/api/core/v1/types.go35-212](https://github.com/kubernetes/kubernetes/blob/2757a872/staging/src/k8s.io/api/core/v1/types.go#L35-L212)
...
-   校验入口：`pkg/apis/core/validation.ValidatePod()`
-   感知特性的字段丢弃：`pkg/apis/core.DropDisabledPodFields()`
-   容器校验：`pkg/apis/core/validation.ValidateContainers()`
-   卷校验：`pkg/apis/core/validation.ValidateVolumes()`

**来源：** [pkg/apis/core/validation/validation.go1-100](https://github.com/kubernetes/kubernetes/blob/2757a872/pkg/apis/core/validation/validation.go#L1-L100) [pkg/features/kube\_features.go1-50](https://github.com/kubernetes/kubernetes/blob/2757a872/pkg/features/kube_features.go#L1-L50)

---

## 特性门控系统

特性门控为 Kubernetes 功能提供演进控制，使特性能够沿着成熟度阶段推进：Alpha → Beta → GA → Locked → Removed。

### 特性门控注册表

所有特性门控定义于 `pkg/features/kube_features.go`：
...
-   API 校验：若门控关闭，通过 `DropDisabledFields()` 丢弃对应字段
-   API 默认值：默认值可能随门控状态变化
-   OpenAPI 生成：门控关闭时对应 schema 字段会被排除
-   组件行为：各组件在运行时查询门控状态
...
Kubernetes 在 API server 与 etcd 之间使用了成熟的缓存层以优化读性能：
...
写操作总是直接落到 etcd，而读操作（Get、List、Watch）在可能的情况下可由 watch cache 提供。缓存通过 Reflector 持续 watch etcd 来保持最新。
...
1.  **HTTP Handler** → 接收请求，执行认证与鉴权
2.  **Feature Gate Check** → 丢弃已禁用特性的字段
3.  **Validation** → 校验对象 schema 与约束
4.  **Admission** → 变更与校验准入 webhook
5.  **Storage** → 持久化到 etcd（写）或从缓存读取（读）
6.  **Watch Notification** → 向订阅者发送变更通知

**来源：** 高层架构图中的图 2，[pkg/apis/core/validation/validation.go410-450](https://github.com/kubernetes/kubernetes/blob/2757a872/pkg/apis/core/validation/validation.go#L410-L450)
...
| --- | --- | --- |
| 构建脚本 | `build/` | 跨平台编译、二进制生成 |
| E2E 框架 | `test/e2e/framework/` | 端到端测试基础设施 |
| 集成测试 | `test/integration/` | 组件集成测试 |
| 依赖管理 | `dependencies.yaml` | 组件版本跟踪 |
| kubeadm | `cmd/kubeadm/` | 集群引导工具 |

**来源：** [build/common.sh](https://github.com/kubernetes/kubernetes/blob/2757a872/build/common.sh) 高层架构图中的图 5

---

## 总结

Kubernetes 代码库是一个大型 monorepo，围绕若干核心组件组织：

-   **Control Plane**：API server、scheduler 与 controllers 负责管理集群状态
-   **Node Components**：kubelet 与 kube-proxy 负责运行工作负载并实现网络能力
-   **API System**：强类型 API 对象体系，集成校验与特性门控
-   **Storage**：分层缓存架构优化 etcd 访问
-   **Build System**：覆盖构建、测试与部署的完整工具链

所有组件都通过中心化 API server 协同工作，后者提供由 etcd 支撑的一致 RESTful 接口。特性门控使整个系统能够逐步演进功能。

关于特定子系统的详细信息，请参见目录中的子页面。
