...
-   [系统组件和数据流](https://github.com/open-webui/open-webui/blob/a7271532/System Components and Data Flow)
-   [前端架构](https://github.com/open-webui/open-webui/blob/a7271532/Frontend Architecture)
-   [后端架构](https://github.com/open-webui/open-webui/blob/a7271532/Backend Architecture)
-   [数据和存储层](https://github.com/open-webui/open-webui/blob/a7271532/Data and Storage Layer)
...
-   **Lines 595-598**: `redis_task_command_listener()` 订阅 Redis 发布/订阅频道以进行跨实例任务取消
-   **Lines 650-655**: `AppConfig` 类包装 `PersistentConfig` 值，启用带有 Redis 缓存的数据库支持配置
-   **Lines 1200-1300**: 中间件栈自底向上处理请求：安全性 → CORS → 会话 → 审计 → 压缩 </old\_str> <new\_str>
...
flowchart TD
    ...
    OpenAIAPIs["OpenAI/Anthropic/etc OPENAI_API_BASE_URLS"]
    WebSearch["Web 搜索引擎 24+ 提供商"]
...
-   **基于组件的前端**：SvelteKit 应用程序具有响应式存储 (`$config`, `$user`, `$models`, `$socket`) 驱动跨组件的 UI 更新
-   **中间件管道**：[backend/open\_webui/utils/middleware.py](https://github.com/open-webui/open-webui/blob/a7271532/backend/open_webui/utils/middleware.py) 中的 `process_chat_payload()` 和 `process_chat_response()` 链接了 RAG、工具、Web 搜索和记忆增强的处理程序
...
flowchart TD
    ...
    MessageInput["MessageInput.svelte 富文本 + 文件"]
    Messages["Messages.svelte 历史树"]
    Stores["Svelte Stores $config, $user, $models"]
    SocketClient["Socket.IO Client $socket"]
    MainPy["main.py FastAPI 应用"]
    MiddlewarePy["utils/middleware.py process_chat_payload()"]
    ...
    SocketMain["socket/main.py Socket.IO 服务器"]
    SQLAlchemy["SQLAlchemy ORM internal/db.py"]
    VectorDBClient["VECTOR_DB_CLIENT retrieval/vector/factory.py"]
    RedisConnection["Redis get_redis_connection()"]
    StorageProvider["Storage storage/provider.py"]
    OllamaAPI["Ollama API OLLAMA_BASE_URLS"]
    OpenAIAPI["OpenAI-compatible OPENAI_API_BASE_URLS"]
    SearchEngines["Web 搜索 15+ 引擎"]
    TTSProviders["TTS/STT OpenAI, Azure, ElevenLabs"]
...
-   **基于组件的前端**：`Chat.svelte` 编排子组件并通过响应式存储进行通信
-   **中间件驱动的后端**：`process_chat_payload()` 函数链接了 RAG、工具、Web 搜索和记忆的处理程序
-   **可插拔数据层**：工厂模式 (`VECTOR_DB_CLIENT`) 和抽象接口 (`StorageProvider`) 允许交换实现
-   **多服务集成**：后端代理并增强对 40+ 个外部 AI 服务的请求
-   **实时通信**：Socket.IO (`socket/main.py`) 为聊天完成和事件提供双向流式传输
...
flowchart TD
    ...
    RootLayout["src/routes/+layout.svelte Auth 检查, store 初始化"]
...
-   `src/app.html`: 在渲染之前注入主题 CSS 变量以防止 FOUC（无样式内容闪烁）
...
状态通过 `$lib/stores` 中定义的 Svelte stores 进行管理：
...
Stores 提供响应式数据流：当 Store 值更改时，所有订阅的组件都会自动重新渲染。
...
├── index.html          # SPA 入口点
├── _app/
│   ├── immutable/      # 哈希资源 (JS, CSS)
│   └── version.json    # 构建版本
└── [other routes]      # 预渲染页面
```
后端通过 `SPAStaticFiles` 提供这些服务，这些服务回退到 `index.html` 以进行客户端路由。
...
后端是一个 FastAPI 应用程序，用于处理 API 请求、编排聊天完成、管理身份验证以及与外部服务交互。

### 请求处理管道

[backend/open\_webui/utils/middleware.py](https://github.com/open-webui/open-webui/blob/a7271532/backend/open_webui/utils/middleware.py) 中的后端中间件实现了一个顺序增强管道，该管道在转发到 LLM 后端之前增强聊天请求：

```mermaid
flowchart TD
    Request["POST /openai/v1/chat/completions {messages, model}"]
    Filter1["Inlet Filters 管道钩子"]
...
| 处理程序 | 位置 | 输入 | 输出 | 用途 |
| --- | --- | --- | --- | --- |
| `process_pipeline_inlet_filter()` | middleware.py:54-56 | `form_data` | 修改后的 `form_data` | 应用自定义管道转换 |
| `chat_memory_handler()` | middleware.py:516-552 | 最后的用户消息 | 带有记忆上下文的系统消息 | 从向量存储中查询记忆 |
| `chat_web_search_handler()` | middleware.py:555-720 | 最后的用户消息 | 带有搜索结果的系统消息 | 生成查询，搜索网页，加载页面 |
| `chat_image_generation_handler()` | middleware.py:723-817 | 最后的用户消息 | 将图像文件添加到消息 | 根据文本提示生成图像 |
| `chat_completion_tools_handler()` | middleware.py:286-513 | 消息 + 工具规范 | 作为上下文的工具结果 | 通过函数调用执行工具 |
...
2.  根据配置有条件地执行（例如 `ENABLE_WEB_SEARCH`）
3.  就地修改 `form_data`（通常添加到系统消息）
4.  在 `extra_params["sources"]` 中累积引用以供 UI 显示
5.  通过 `__event_emitter__` Socket.IO 回调发出状态更新

### 路由器组织和端点

后端 API 组织在 `backend/open_webui/routers/` 下的路由器中：
...
| `retrieval.py` | `/api/retrieval` | `POST /process/web`, `POST /process/youtube`, `GET /config`, `POST /config/update` | RAG 配置，Web/文档处理 |
| `audio.py` | `/api/audio` | `POST /transcriptions`, `POST /speech` | 语音转文本 (STT) 和文本转语音 (TTS) |
| `images.py` | `/api/images` | `POST /generations`, `POST /edits` | 图像生成和编辑 |
| `tools.py` | `/api/tools` | `GET /`, `POST /create`, `POST /{id}/update` | 工具定义和执行 |
...
| `pipelines.py` | `/api/pipelines` | `GET /`, `POST /upload`, `POST /{id}/valves/update` | 外部管道过滤器 |
...
Open WebUI 使用 SQLAlchemy ORM 进行关系数据存储，支持 SQLite（默认）、PostgreSQL 和 MySQL：
...
flowchart TD
    ...
    UsersClass["Users class users.py"]
    ChatsClass["Chats class chats.py"]
    FilesClass["Files class files.py"]
    FunctionsClass["Functions class functions.py"]
...
-   **表模型**: SQLAlchemy 声明式类（例如 `User`, `Chat`, `File`）映射到数据库表
-   **业务逻辑类**: 包装类（例如 `Users`, `Chats`, `Files`）提供用于 CRUD 操作的静态方法
...
| `file` | `Files` | 上传的文件元数据，对存储的引用 |
| `function` | `Functions` | 用户定义的 Python 函数（管理员创建） |
| `tool` | `Tools` | 带有 OpenAPI 规范的工具定义 |
| `model` | `Models` | LLM 模型配置，元数据，过滤器 |
| `knowledge` | `Knowledges` | 用于 RAG 的知识库元数据 |
| `config` | N/A (直接访问) | 持久化应用程序配置（数据库支持） |
...
| 方法 | 路由器端点 | 实现 | 令牌创建 |
| --- | --- | --- | --- |
| **Local** | `POST /api/auths/signin` | `Users.authenticate_user()` → `Auths.authenticate_user()` | [backend/open\_webui/utils/auth.py120-150](https://github.com/open-webui/open-webui/blob/a7271532/backend/open_webui/utils/auth.py#L120-L150) 中的 `create_token()` |
| **LDAP** | `POST /api/auths/signin` | `ldap3.Connection()` verify → `Users.insert_new_user()` | 相同的 JWT 创建 |
| **OAuth/OIDC** | `GET /api/auths/oauth/{provider}/callback` | `OAuthManager.authorize_access_token()` | 相同的 JWT 创建 |
| **Trusted Header** | Any endpoint | `WEBUI_AUTH_TRUSTED_EMAIL_HEADER` 环境检查 | 自动用户查找/创建 |
| **API Key** | Any endpoint | `Auths.get_user_by_api_key()` | 无令牌；每个请求验证密钥 |
...
3.  [backend/open\_webui/utils/auth.py150-180](https://github.com/open-webui/open-webui/blob/a7271532/backend/open_webui/utils/auth.py#L150-L180) 中的 `decode_token()` 验证 JWT 签名和过期
4.  返回带有 `role` 字段（`"admin"`, `"user"`, `"pending"`）的 `UserModel`
5.  路由可以检查 `user.role` 或使用 `get_admin_user()` 依赖项用于仅限管理员的端点
...
flowchart TD
    ...
    Chroma["ChromaVectorDB vector/chroma.py 默认"]
...
`VECTOR_DB_CLIENT` 全局单例基于 `VECTOR_DB` 环境变量进行初始化。所有实现都公开一个通用接口：
...
| 数据库 | 配置环境变量 | 用例 |
| --- | --- | --- |
| ChromaDB (默认) | `CHROMA_DATA_PATH`, `CHROMA_TENANT`, `CHROMA_DATABASE` | 开发，单实例 |
| Milvus | `MILVUS_URI` | 生产，高可扩展性 |
| Qdrant | `QDRANT_URI`, `QDRANT_API_KEY` | 生产，云原生 |
| OpenSearch | `OPENSEARCH_URI`, `OPENSEARCH_SSL` | 企业，AWS 集成 |
| PGVector | `DATABASE_URL` (PostgreSQL with pgvector) | PostgreSQL 用户 |
...
Open WebUI 通过 [backend/open\_webui/retrieval/vector/factory.py1-100](https://github.com/open-webui/open-webui/blob/a7271532/backend/open_webui/retrieval/vector/factory.py#L1-L100) 中的工厂模式支持多个向量存储：
...
```
每个实现类都包装底层的向量数据库客户端并提供：

-   **集合管理**：创建/删除集合（每个文件或知识库一个）
-   **文档摄取**：`upsert()` 存储带有元数据（文件 ID、块索引等）的嵌入
-   **相似度搜索**：`search()` 返回余弦相似度最高的 top-k 近邻
-   **混合搜索**：一些实现将向量搜索与 BM25 关键字搜索相结合
...
| --- | --- | --- | --- |
| **Sessions** | `{prefix}:session:{session_id}` | 跨实例存储会话数据 | [backend/open\_webui/main.py1250-1270](https://github.com/open-webui/open-webui/blob/a7271532/backend/open_webui/main.py#L1250-L1270) |
| **Config Sync** | `{prefix}:config:{key}` | 广播配置更改 | [backend/open\_webui/config.py258-261](https://github.com/open-webui/open-webui/blob/a7271532/backend/open_webui/config.py#L258-L261) |
| **Task Queue** | `{prefix}:commands` (pub/sub channel) | 跨实例取消任务 | [backend/open\_webui/tasks.py100-150](https://github.com/open-webui/open-webui/blob/a7271532/backend/open_webui/tasks.py#L100-L150) |
| **Yjs CRDT** | `ydoc:{doc_id}` (list) | 存储协作文档更新 | [backend/open\_webui/models/notes.py50-100](https://github.com/open-webui/open-webui/blob/a7271532/backend/open_webui/models/notes.py#L50-L100) |
...
| --- | --- | --- | --- | --- |
| **Local** | `UPLOAD_DIR` | N/A | 文件存储在 `{UPLOAD_DIR}/{file_id}` | 不支持 |
| **S3** | `S3_BUCKET_NAME`, `S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY` | S3 存储桶 (AWS, MinIO, DigitalOcean) | 也保存到 `{UPLOAD_DIR}/{file_id}` | `boto3.generate_presigned_url()` |
| **GCS** | `GCS_BUCKET_NAME`, `GOOGLE_APPLICATION_CREDENTIALS` | Google Cloud Storage 存储桶 | 也保存到 `{UPLOAD_DIR}/{file_id}` | `blob.generate_signed_url()` |
| **Azure** | `AZURE_STORAGE_ACCOUNT_NAME`, `AZURE_STORAGE_CONNECTION_STRING` | Azure Blob Storage 容器 | 也保存到 `{UPLOAD_DIR}/{file_id}` | `generate_blob_sas()` |

云提供商使用 **双重存储策略**：文件上传到远程对象存储以确保持久性，然后下载并缓存在本地以进行快速检索。这允许应用程序直接从磁盘提供文件服务，同时维护云备份。
...
所有 HTTP 请求都包含 `Authorization: Bearer {token}` 标头。[backend/open\_webui/utils/auth.py180-220](https://github.com/open-webui/open-webui/blob/a7271532/backend/open_webui/utils/auth.py#L180-L220) 中的 `get_verified_user()` 依赖项验证 JWT 令牌并将 `UserModel` 注入路由处理程序。
...
| `"events"` | `{type: "source", data: {name: "...", document: "..."}}` | `chat_completion_files_handler()` | 添加引用 |
| `"events"` | `{type: "chat:title", data: "..."}` | `generate_title()` | 更新聊天标题 |
| `"events"` | `{type: "usage", data: {model: "...", tokens: ...}}` | `generate_chat_completion()` | 跟踪使用情况 |
...
-   `USER_POOL`: `{user_id: [socket_ids]}` - 将用户映射到他们的连接（支持多标签页）
-   `USAGE_POOL`: `{socket_id: {model_id: timestamp}}` - 跟踪每个连接的活跃 LLM 使用情况

当配置 Redis 时，这些变成 `RedisDict` 实例以进行跨实例同步。

### 后端到 LLM 代理模式

后端充当 LLM API 的增强反向代理：
...
`chat_completion_files_handler()` [backend/open\_webui/utils/middleware.py820-1045](https://github.com/open-webui/open-webui/blob/a7271532/backend/open_webui/utils/middleware.py#L820-L1045) 中的 RAG 管道步骤：
...
5.  **检索** (lines 995-1020):
    ...
    -   混合搜索将 BM25 关键字搜索与向量相似度相结合
    -   如果已配置，则通过交叉编码器应用重排序
6.  **上下文注入** (lines 1025-1045):

    -   使用 `RAG_TEMPLATE`（可通过管理设置配置）格式化检索到的段落
    -   通过 `add_or_update_system_message()` 注入系统消息
    -   将引用添加到 `extra_params["sources"]` 以供 UI 显示
...
flowchart TD
    ...
    Publish["Redis SET"]
    Notify["其他实例读取"]
    EnvVars["Environment Variables .env 文件"]
...
[backend/open\_webui/config.py165-222](https://github.com/open-webui/open-webui/blob/a7271532/backend/open_webui/config.py#L165-L222) 中的 `PersistentConfig` 类包装了配置值：

    -   从环境变量读取作为默认值
    -   如果存在，则使用数据库值覆盖
    -   在全局 `PERSISTENT_CONFIG_REGISTRY` 中注册以进行更新
...
3.  其他实例的 `AppConfig.__getattr__` 读取更新后的值
4.  使用配置值的组件自动查看更改
...
Open WebUI 的架构旨在实现模块化、可扩展性和可扩展性：

-   **三层分离**：表示层 (SvelteKit)、应用层 (FastAPI)、数据层 (SQL/Vector/Redis/Storage)
-   **中间件管道**：顺序处理程序使用记忆、搜索、工具和 RAG 增强聊天请求
-   **可插拔组件**：交换数据库、向量存储、存储提供者和 LLM 后端
-   **实时通信**：Socket.IO 支持流式传输响应和协作功能
-   **分布式配置**：Redis 支持的 AppConfig 允许跨实例进行零停机更新
