...
本文档描述了 Open WebUI 的 Python 依赖管理系统，包括依赖的组织方式、按子系统的分类、版本锁定策略以及安装方法。本文档仅涵盖后端 Python 依赖。有关前端依赖的信息，请参阅前端构建系统文档。

**相关页面：**

-   有关使用这些依赖的环境配置，请参阅[环境配置](/open-webui/open-webui/9.3-collaborative-editing)
-   有关使用数据库相关依赖的数据库设置，请参阅[数据库配置](/open-webui/open-webui/17.3-optional-dependencies)
-   有关依赖于云端 SDK 的存储提供者实现，请参阅[存储提供者系统](/open-webui/open-webui/12.1-storage-provider-architecture)
...
Open WebUI 使用多个文件来组织和管理 Python 依赖，支持不同的安装方法和用例。

### 依赖文件

```mermaid
flowchart TD
    reqtxt["requirements.txt 153 个依赖 完整安装"]
    pyproject["pyproject.toml PEP 621 格式 包元数据"]
    reqmin["requirements-min.txt 52 个依赖 最小运行时"]
    uvlock["uv.lock 解析后的依赖树 可复现的构建"]
    postgres["[postgres] psycopg2-binary pgvector"]
    all["[all] 测试工具 额外的向量数据库 云端仿真器"]
    Install1["标准安装"]
    Install2["包安装"]
    Install3["最小化安装"]
    Install4["快速安装"]
...
| 文件 | 用途 | 用例 |
| --- | --- | --- |
| `requirements.txt` | 包含精确版本的完整依赖列表 | 生产环境部署、Docker 镜像 |
| `pyproject.toml` | Python 包元数据和依赖 | 包安装、开发 |
| `requirements-min.txt` | 实现基础功能的最小依赖 | 轻量级部署、测试 |
| `uv.lock` | 带有哈希值的已解析依赖树 | 使用 `uv` 包管理器的可复现构建 |
...
-   **python-jose==3.5.0**：JWT 令牌创建与验证
-   **cryptography**：低级密码学原语（传递依赖，也由云提供商使用）
-   **bcrypt==5.0.0**：使用 bcrypt 算法的密码哈希
...
-   **sqlalchemy==2.0.38**：用于复杂查询和 PostgreSQL 支持的主要 ORM
-   **alembic==1.17.2**：用于 SQLAlchemy 的数据库迁移工具
-   **peewee==3.18.3**：用于 SQLite 操作和遗留代码的轻量级 ORM
-   **peewee-migrate==1.14.3**：用于 Peewee 的迁移工具
...
-   **redis**：用于会话管理和发布/订阅的内存数据存储
-   **pycrdt==0.12.25**：用于 Yjs 文档协作的 CRDT (无冲突复制数据类型) 实现
-   **starsessions\[redis\]==2.2.1**：具有 Redis 后端的会话中间件
...
-   **requests==2.32.5**：用于简单请求的同步 HTTP 客户端
-   **aiohttp==3.12.15**：用于并发请求的异步 HTTP 客户端
-   **httpx\[socks,http2,zstd,cli,brotli\]==0.28.1**：具有高级特性的现代异步/同步 HTTP 客户端：
    -   `socks`：SOCKS 代理支持
    -   `http2`：HTTP/2 协议支持
    -   `zstd`：Zstandard 压缩
    -   `cli`：命令行界面
...
-   **async-timeout**：用于 asyncio 的超时上下文管理器
-   **aiocache**：异步缓存框架
...
-   **tiktoken**：OpenAI 的分词器，用于 token 计数
...
-   **langchain==0.3.27**：LLM 应用程序链框架
-   **langchain-community==0.3.29**：社区贡献的集成
...
-   **APScheduler==3.10.4**：用于定期任务的类似 Cron 的作业调度
-   **RestrictedPython==8.0**：用于用户提供代码的沙箱化 Python 执行
...
-   **loguru==0.7.3**：增强型日志记录，具有彩色输出和结构化日志
...
可选依赖被组织成不同的组，可以根据部署要求分别安装。

### PostgreSQL 支持

`postgres` 可选依赖组添加了针对 PostgreSQL 的驱动程序：
...
```
-   **psycopg2-binary==2.9.10**：用于 Python 的 PostgreSQL 适配器（为了安装方便使用二进制分发版）
-   **pgvector==0.4.1**：PostgreSQL 扩展支持，用于向量相似度搜索
...
# 或者
pip install -r requirements.txt  # 在主文件中已包含这些
```
...
云存储提供者支持是模块化的，每个提供者都需要特定的 SDK 包。
...
-   **boto3==1.41.5**：用于 Python 的 AWS SDK，提供 S3 客户端
    -   用于 [backend/open\_webui/storage/provider.py108-224](https://github.com/open-webui/open-webui/blob/a7271532/backend/open_webui/storage/provider.py#L108-L224)
    -   在未提供凭据时支持工作负载身份 (IAM 角色)
    -   配置包括加速端点和寻址样式选项

### Google Cloud Storage 依赖

-   **google-cloud-storage==2.19.0**：GCS 客户端库
    -   用于 [backend/open\_webui/storage/provider.py226-290](https://github.com/open-webui/open-webui/blob/a7271532/backend/open_webui/storage/provider.py#L226-L290)
    -   支持服务账号 JSON 或默认凭据
-   **google-api-python-client**：Google API 客户端（也用于 Google Drive 集成）
-   **google-auth-httplib2**：用于 Google 认证的 HTTP 库
-   **google-auth-oauthlib**：针对 Google 服务的 OAuth2 流程
...
-   **azure-storage-blob==12.24.1**：Azure Blob Storage 客户端
    -   用于 [backend/open\_webui/storage/provider.py292-360](https://github.com/open-webui/open-webui/blob/a7271532/backend/open_webui/storage/provider.py#L292-L360)
    -   同时支持显式密钥和 DefaultAzureCredential (托管身份)
-   **azure-identity==1.25.0**：支持多种凭据类型的 Azure 身份验证
...
Open WebUI 支持多种向量数据库以实现 RAG 功能。除了 ChromaDB（默认）之外，大多数都是可选的。
...
flowchart TD
    ...
    azureSearch["azure-search-documents==11.6.0 Azure AI Search"]
    milvus["pymilvus==2.6.4 高性能"]
    oracledb["oracledb==3.2.0 Oracle 23ai 向量"]
    elasticsearch["elasticsearch==9.1.0 搜索 + 向量"]
    colbert["colbert-ai==0.2.21 后交互重排序"]
    rankbm25["rank-bm25==0.2.2 BM25 关键字搜索"]
    RAGPipeline["RAG 管道"]
    Reranking["结果重排序"]
    HybridSearch["混合搜索"]
...
-   **chromadb==1.1.0**：默认的嵌入式向量数据库，始终安装
    -   内存或持久化存储
    -   原生支持嵌入
    -   无需外部服务

**注意：** requirements.txt 中版本锁定为 1.1.0，但 pyproject.toml 中为 1.0.20。requirements.txt 中的版本优先。
...
-   **opensearch-py==2.8.0**：具有向量支持的 OpenSearch
-   **pymilvus==2.6.4**：Milvus 高性能向量数据库
-   **qdrant-client==1.14.3**：Qdrant 向量搜索引擎
-   **pinecone==6.0.2**：Pinecone 托管向量数据库
-   **oracledb==3.2.0**：具有向量支持的 Oracle Database 23ai
-   **elasticsearch==9.1.0**：具有向量搜索功能的 Elasticsearch（仅在 `all` 组中）
-   **azure-search-documents==11.6.0**：Azure AI Search（仅在 `all` 组中）
...
-   **rank-bm25==0.2.2**：用于基于关键字的混合搜索的 BM25 算法
-   **colbert-ai==0.2.21**：用于重排序的 ColBERT 后交互模型
...
-   **opencv-python-headless==4.11.0.86**：不带 GUI 依赖的计算机视觉
-   **rapidocr-onnxruntime==1.4.4**：使用 ONNX 运行时的快速 OCR
-   **onnxruntime==1.20.1**：ONNX 模型推理运行时
-   **azure-ai-documentintelligence==1.0.2**：Azure Document Intelligence OCR/提取
...
-   **soundfile==0.13.1**：读/写音频文件
-   **pydub**：音频操作和转换
-   **faster-whisper==1.1.1**：优化的 Whisper 语音识别
-   **av==14.0.1**：用于音频/视频容器操作的 PyAV
    -   由于 FIPS 自检失败问题而锁定版本（见 [backend/requirements.txt125](https://github.com/open-webui/open-webui/blob/a7271532/backend/requirements.txt#L125-L125)）
...
-   **playwright==1.56.0**：针对 JavaScript 密集型站点的浏览器自动化
...
-   **pytest~=8.4.1**：测试框架
-   **pytest-docker~=3.2.5**：pytest fixture 中的 Docker 容器
-   **pytest-asyncio>=1.0.0**：异步测试支持（在 dev 组中）
...
这些包含在用于测试云端集成的 `all` 可选组中：

-   **moto\[s3\]>=5.0.26**：AWS 服务模拟 (S3 等)
-   **gcp-storage-emulator>=2024.8.3**：Google Cloud Storage 仿真器
...
-   **ldap3==2.9.1**：用于 Active Directory 集成的纯 Python LDAP 客户端
...
chromadb==1.1.0
```
这确保了跨部署行为的一致性，并防止因依赖更新而导致的意外中断。
...
1.  不太可能发生破坏性变更的稳定 API (例如 `redis`)
2.  由其他包的版本锁定进行管理 (例如被 `PyJWT[crypto]` 锁定的 `cryptography`)
3.  最新版本即可满足要求的可选功能
...
该项目需要 Python 3.11 或 3.12 (不包括预览版)：
...
```
这将安装来自 requirements.txt 的所有 153 个包，包括向量数据库和云存储 SDK 等可选依赖项。
...
uv pip install .
```
`uv.lock` 文件包含已解析的依赖版本以及用于验证的加密哈希，确保跨平台的可复现构建。
...
`pyproject.toml` 使用 Hatchling 定义构建系统：
...
```
版本号从 `package.json` 读取，以保持与前端包版本一致。
...
该依赖管理系统为不同的部署场景提供了灵活性，同时通过版本锁定和锁定文件保持了可复现性。模块化的可选依赖项允许在资源受限的环境中进行最小化安装，同时支持启用所有集成的全功能部署。
