...
本文档描述了后端中间件系统，该系统在聊天完成 (chat completion) 请求到达 LLM 提供商之前拦截并增强这些请求。该管道通过向提示词 (prompts) 中注入额外的上下文，实现了检索增强生成 (RAG)、工具执行、Web 搜索和用户记忆等功能。

有关发起这些请求的前端聊天系统的信息，请参阅[聊天系统](/open-webui/open-webui/3-installation-and-deployment)。有关 RAG 文档处理和向量存储子系统的详细信息，请参阅 [RAG 和知识系统](/open-webui/open-webui/6-backend-processing-pipeline)。有关 LLM 提供商集成和路由的信息，请参阅[模型管理](/open-webui/open-webui/11-configuration-management)。

## 管道概览

后端处理管道实现为一个中间件层，位于聊天 API 端点和 LLM 提供商代理路由器之间。当接收到聊天完成请求时，它会通过多个处理函数，这些函数可以在将请求转发给 LLM 之前修改请求载荷 (payload)。
...
每个处理程序都根据配置标志、用户权限和请求元数据有条件地调用。如果处理程序直接生成响应（例如图像生成），该管道支持提前终止。

| 处理程序 | 配置标志 | 触发条件 |
| --- | --- | --- |
| 记忆 (Memory) | 始终启用 | 用户具有记忆权限 |
| 文件 (RAG) | N/A | 请求元数据中存在 `files` |
| 工具 (Tools) | `ENABLE_RAG_WEB_LOADER_SSL_VERIFICATION` | 请求体中存在 `tool_ids` |
| Web 搜索 | `ENABLE_WEB_SEARCH` | 模型启用了 Web 搜索 |
| 图像生成 | `ENABLE_IMAGE_GENERATION` | 请求带有图像生成标签 |
| 过滤器 (Filters) | N/A | 用户分配了过滤器 |
...
[backend/open_webui/utils/middleware.py488-1090](https://github.com/open-webui/open-webui/blob/a7271532/backend/open_webui/utils/middleware.py#L488-L1090) 中的 `process_chat_payload` 函数是处理管道的中心编排器。它接收聊天完成请求、用户上下文和应用程序状态，然后依次调用各处理程序。
...
-   `form_data`: 包含 `messages`、`model` 和可选 `metadata` 的聊天完成请求体
-   `user`: 发起请求的已认证用户
-   `metadata`: 请求元数据，包括 `chat_id`、`message_id`、`session_id` 和功能标志
-   返回值：`(modified_form_data, extra_params)` 元组，其中 `extra_params` 包含 `sources` 和其他处理程序的输出
...
### 事件发射器 (Event Emitter) 和调用者模式

该管道使用事件驱动架构将状态更新和结果实时流式传输回客户端：

```mermaid
flowchart TD
    Handler["处理函数"]
    EventEmitter["event_emitter AsyncGenerator"]
...
`chat_memory_handler` 函数从用户的记忆库中检索相关的过去上下文，并将其注入到系统消息中。这发生在所有其他处理程序之前，以提供一致的背景上下文。
...
```
处理程序根据与当前用户消息的语义相似度，查询最多 `k=3` 条最相关的记忆文档。每条记忆条目都包含创建时间戳，以提供时间上下文。
...
文件处理程序通过处理附加文档、提取相关分块并将其注入提示词中来实现检索增强生成 (RAG)。这是管道中最复杂的处理程序。
...
对于存储在向量数据库中的文件，系统使用任务模型根据用户消息生成多个优化的搜索查询：
...
```
任务模型分析对话历史并生成 1-5 个搜索查询，这些查询捕捉了信息需求的不同方面。与使用原始用户消息相比，这提高了检索质量。
...
### 混合搜索和重排序 (Hybrid Search and Reranking)

文档检索结合了向量相似度搜索和 BM25 关键字匹配：

```mermaid
flowchart TD
    Query["搜索查询"]
    Vector["VectorSearchRetriever 嵌入相似度"]
    BM25["BM25Retriever 关键字匹配"]
...
-   `TOP_K`: 要检索的文档数量（默认值：4）
-   `TOP_K_RERANKER`: 在选择前 k 个之前进行重排序的数量（默认值：4）
-   `RELEVANCE_THRESHOLD`: 最低相似度分数 (0.0-1.0)
-   `HYBRID_BM25_WEIGHT`: BM25 和向量搜索之间的平衡 (0.0-1.0)
...
检索到的文档使用可配置的模板进行格式化，并注入到第一条用户消息中：
...
```
上下文被前置到用户消息内容中，而来源被单独跟踪以便在 UI 中显示引用。
...
工具处理程序实现了一个两阶段执行模型：首先，任务模型根据可用的规范生成结构化的工具调用；其次，系统执行这些调用并为 LLM 格式化结果。
...
| 外部 (OpenAPI) | HTTP 请求 | REST API 工具服务器 |
| MCP | 模型上下文协议 | 结构化工具服务器 |
...
1.  **HTML 响应处理：** 如果工具返回带有 `Content-Disposition: inline` 的 `HTMLResponse`，则 HTML 内容会被提取为嵌入项，并生成成功/错误消息
2.  **文件提取：** 数据 URL 和 base64 图像会被提取并转换为文件引用
3.  **来源构建：** 结果被格式化为带有元数据的来源，以便进行引用
...
Web 搜索处理程序生成搜索查询，通过配置的搜索引擎执行搜索，并可选地加载和嵌入结果页面的完整内容。
...
该系统通过插件架构支持 20 多种搜索引擎。每种引擎都实现了一个 `search_*` 函数，该函数返回一个 `SearchResult` 对象：
...
当禁用 `BYPASS_WEB_SEARCH_WEB_LOADER`（默认）时，系统会加载完整的页面内容：

```mermaid
flowchart TD
    URLs["搜索结果 URL"]
    LoaderEngine["WEB_LOADER_ENGINE"]
    Playwright["PlaywrightWebLoader JavaScript 渲染"]
    ...
    TextSplit["文本分割 RecursiveCharacterTextSplitter"]
...
-   `WEB_SEARCH_CONCURRENT_REQUESTS`: 并行搜索查询的数量（默认值：10）
-   `WEB_LOADER_CONCURRENT_REQUESTS`: 并行页面加载的数量（默认值：10）
-   `WEB_SEARCH_RESULT_COUNT`: 每个查询的结果数量（默认值：3）
...
Web 搜索结果以两种格式之一添加到 `form_data["files"]` 中：
...
```
这些条目随后由文件处理程序使用与上传文档相同的 RAG 管道进行处理。
...
完整的管道按顺序整合了所有处理程序，每个处理程序都有机会修改请求并添加上下文：

```mermaid
flowchart TD
    Request["传入的聊天请求 {messages, model, metadata}"]
    Memory["记忆处理程序 用户上下文 → 系统消息"]
    Files["文件处理程序 RAG 上下文 → 用户消息"]
    Tools["工具处理程序 工具结果 → 来源"]
    WebSearch["Web 搜索处理程序 搜索结果 → 文件"]
    ImageGen["图像生成处理程序 生成的图像 → 文件"]
    Filters["过滤器函数 自定义入站/出站过滤器"]
...
处理程序通过 `process_chat_payload` 返回的 `extra_params` 字典来传递结果：
...
管道完成后，修改后的请求被发送到 LLM 提供商。响应通过 `process_chat_response` 中间件流式回传，该中间件：

1.  向 WebSocket 客户端发送 token 增量
2.  更新数据库中的消息内容
3.  注入来源引用
4.  处理思维链 (chain-of-thought) 模型的推理标签
5.  触发后续问题的生成
...
每个处理程序的行为都由存储在 [backend/open_webui/config.py](https://github.com/open-webui/open-webui/blob/a7271532/backend/open_webui/config.py) 中的应用程序配置控制：
...
| `TOP_K` | 检索的文档数量 | 4 |
| `TOP_K_RERANKER` | 重排序的文档数量 | 4 |
| `RELEVANCE_THRESHOLD` | 最低相似度分数 | 0.0 |
| `ENABLE_RAG_HYBRID_SEARCH` | 使用 BM25 + 向量搜索 | False |
| `HYBRID_BM25_WEIGHT` | 混合搜索中的 BM25 权重 | 0.5 |
...
| 设置 | 描述 | 默认值 |
| --- | --- | --- |
| `ENABLE_WEB_SEARCH` | 启用 Web 搜索 | False |
...
每个处理程序都封装在 try-except 块中，以防止单个处理程序破坏整个管道：
...
当外部服务（Web 搜索、文档加载）失败时，系统会优雅降级：

-   Web 搜索失败 → 继续处理，不带 Web 结果
-   工具执行失败 → 向 LLM 返回错误消息
-   RAG 查询失败 → 回退到原始文档或在无上下文的情况下继续
