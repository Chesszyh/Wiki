...
本文档描述了 Open WebUI 中的实时通信系统，该系统使用 Socket.IO 在前端和后端之间实现了双向通信，并可选配 Redis 后端以支持分布式部署。该系统支持实时聊天流、协作式文档编辑、会话管理以及分布式任务协调。
...
flowchart TD
    ...
    SocketStore["Socket Store stores/index.ts:socket"]
...
| 房间模式 | 用途 | 成员 |
| --- | --- | --- |
| `user:{user_id}` | 用户特定事件 | 该用户的所有会话 |
| `channel:{channel_id}` | 频道消息 | 该频道的所有成员 |
| `doc_{document_id}` | 协作编辑 | 该文档的所有编辑器 |
...
前端注册了两个主要的事件处理程序，用于处理来自后端的入站事件：
...
| `embeds` | 扩展 embeds 数组 |
| `files` | 扩展 files 数组 |
| `source` / `citation` | 追加到 sources 数组 |
...
`SESSION_POOL` 维护 Socket ID 到用户信息的映射，以便进行快速的身份验证和授权检查。
...
flowchart TD
    ...
    GetRoom["get_user_ids_from_room(): 获取房间内的所有 sid 并映射到用户 ID"]
...
`USAGE_POOL` 追踪哪些会话正在活跃使用哪些模型，并自动清理过期的条目。
...
```mermaid
flowchart TD
    SetItem["setitem(key, value) 序列化为 JSON 并执行 HSET name key value"]
    GetItem["getitem(key) HGET name key 并反序列化 JSON"]
    DelItem["delitem(key) HDEL name key"]
    Contains["contains(key) HEXISTS name key"]
    Keys["keys() HKEYS name"]
    Values["values() HVALS name 并反序列化所有"]
    Items["items() HGETALL name 并反序列化所有"]
    HSET["HSET"]
    HGET["HGET"]
    HDEL["HDEL"]
    HEXISTS["HEXISTS"]
...
-   将值序列化为 JSON 以便存储在 Redis 中。
-   支持标准字典操作 (`[]`, `in`, `keys()`, `values()`, `items()`)。
-   提供用于原子替换的 `set()` 和用于批量操作的 `update()`。

## 协作文档编辑

系统使用 Yjs CRDT (无冲突复制数据类型) 和 Redis 后端存储实现了实时协作编辑。
...
flowchart TD
    ...
    RemoveUser["remove_user() SREM 从 users 集合中移除"]
    Clear["clear_document() 删除这两个键"]
...
-   **自动清理**：当最后一名用户离开时，文档将被清理（删除两个键）。
-   **回退机制**：如果 Redis 不可用，则使用内存字典。

### 感知 (Awareness) 更新

除了文档更新外，系统还支持不进行持久化的感知信息（光标位置、选区）：
...
系统使用 Redis 发布/订阅提供分布式任务协调功能，允许任何服务器实例取消在任何其他实例上运行的任务。
...
flowchart TD
    ...
    PubSubChannel["{prefix}:tasks:commands PUBLISH 频道 消息: {'action': 'stop', 'task_id': ...}"]
...
每个服务器实例都运行一个后台监听器，用于监听任务命令：

```mermaid
flowchart TD
    Startup["FastAPI 寿命 (Lifespan) 启动"]
    Listener["redis_task_command_listener()"]
    Subscribe["pubsub.subscribe(REDIS_PUBSUB_CHANNEL)"]
    Listen["遍历 pubsub.listen() 的异步循环"]
...
flowchart TD
    ...
    FailoverLogic["连接错误重试，只读错误重试，最大重试次数: SENTINEL_MAX_RETRY_COUNT"]
...
`SentinelRedisProxy` 提供带有重试逻辑的自动故障转移：

| 错误类型 | 原因 | 操作 |
| --- | --- | --- |
| `ConnectionError` | 主节点下线 | 从 Sentinel 获取新主节点并重试 |
| `ReadOnlyError` | 故障转移后写入旧主节点 | 使用新主节点重试 |

重试配置：

-   最大重试次数：`REDIS_SENTINEL_MAX_RETRY_COUNT` (默认从环境获取)
...
flowchart TD
    ...
    SentinelCheck["是否存在 WEBSOCKET_SENTINEL_HOSTS？"]
...
-   通过 `sio.emit()` 发出的事件会被发布到 Redis 发布/订阅中。
-   订阅了同一频道的所有服务器实例都会收到该事件，并将其传递给其连接的客户端。
...
-   `aquire_lock()`：设置带有 `NX` (仅在不存在时) 和 `EX` (过期时间) 的键。
-   `renew_lock()`：设置带有 `XX` (仅在存在时) 的键以延长过期时间。
-   `release_lock()`：仅当值匹配锁 ID 时才删除键 (防止释放另一个进程的锁)。
...
| `REDIS_KEY_PREFIX` | Redis 键前缀 | \- | 字符串前缀 |
